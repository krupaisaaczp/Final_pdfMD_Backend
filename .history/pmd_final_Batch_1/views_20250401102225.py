...
def analyze_pdf(request):
    logger.debug(f"Analyze PDF request headers: {request.headers}")
    logger.debug(f"Analyze PDF request files: {request.FILES} by {request.user.username if request.user.is_authenticated else 'unauthenticated'}")
    if not request.user.is_authenticated:
        logger.warning(f"Unauthorized attempt to analyze PDF by {request.META.get('REMOTE_ADDR')}")
