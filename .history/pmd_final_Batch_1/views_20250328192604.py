...
class PDFFileViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            report_filename = f"{unique_id}_{file.name}.pdf"
            report_path = generate_report(str(absolute_file_path), report_filename)

            # ✅ Step 6: Explain the Prediction
            explanation_filename = f"{unique_id}_{file.name}_explanation.txt"
            explanation_path = explain_prediction(str(absolute_file_path), explanation_filename)
            ...
            return Response({
                ...
                "report_path": str(Path(settings.MEDIA_URL) / report_filename),
                "explanation_path": str(Path(settings.MEDIA_URL) / explanation_filename),
